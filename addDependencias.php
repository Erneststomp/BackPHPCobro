...
if ($usuarioInicioSesion==true || $result1->num_rows == 0) {
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        if (!empty($data)) {
            ...
            try {
                $sqlVerificarUnidad = "SELECT COUNT(*) AS count FROM $nombreTabla WHERE dependencias = '$dependencia'";
                ...
                if ($result->num_rows > 0) {
                    ...
                    if ($row["count"] > 0) {
                        $out = [
                            "result" => false,
                            "msg" => "Ya existe esa dependencia"
                        ];
                    } else {
                        // No se encontró un usuario con los mismos datos, proceder con la inserción.
                        $sql2 = "INSERT INTO $nombreTabla (dependencias,estado)"
                            . "VALUES ('$dependencia','active')";
